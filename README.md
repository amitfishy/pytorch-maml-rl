# Robustness  of  MAML  to  Drastic  Task  Variations  and  Improving  Task Sampling  Efficiency  for  various  Gym  Environments


This repo contains the code for the Comp767-RL course and is based on the original repo described below. The only additional installation requirement is a small change in the gym library that is detailed in this [issue](https://github.com/tristandeleu/pytorch-maml-rl/issues/28).

Otherwise the only other addition is [comet_ml](https://www.comet.ml/) to keep track of things during training.

Experiments carried out in the report are run by executing the bash script with `./run_experiments.sh`. Detailled usage instructions can be seen by using `python main.py --help`, particularly for the details of all the arguments to be given. Models are saved in the folders specified in the commands and the graphs can be generated by using `python plot_baselines.py` (after specifying the folder name in the script for which graphs are to be generated).
